...
class SeekerJobController extends Controller
{
    ...
    public function allSeekers()
    {
        return inertia('Backend/Seekers/Index', [
            ...
            'seekers' => User::query()
                ...
                ->through(fn($seeker) => [
                    'id' => $seeker->id,
                    'profile_complete' => $seeker->profileComplete(),
                    'name' => $seeker->name,
                    'phone' => $seeker->phone,
                    'photo' => $seeker->photo,
                    'email' => $seeker->email,
    ...
    public function singleSeeker($id){
        ...
        $user        = User::findOrFail($id)->load(['seeker', 'seeker.division',
            'seeker.district', 'seeker.educaiton', 'seeker.category',
            'seeker.education_level', 'seeker.category']);
